...
import { API_BASE_URL, LOGIN_IMAGE_API, REGISTER_USER, LOGIN_USER } from './Constants';
import LoginImage from '../../src/assets/LoginImage.jpg';

// Set to false when API is not available
...
export const loginUser = async (email, password) => {
  ...
  if (API_AVAILABLE) {
    try {
      const response = await axios.post(`${API_BASE_URL}${LOGIN_USER}`, { email, password });
...
export const sendResetPasswordEmail = async (email) => {
  if (API_AVAILABLE) {
    ...
    // Handle alternative action or display error message
  }
};